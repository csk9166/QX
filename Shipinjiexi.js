// ==UserScript==
// @name 通用VIP视频解析
// @author YTG自用修改版.Safari浏览器视频解析脚本，配合Injectscript使用
// @version 2.3.0
// @description 爱奇艺、腾讯视频、优酷、乐视、芒果TV、搜狐、1905、PPTV、B站。
// @include *
// @match *
// @createTime 2021-03-03 12:00:47
// @updateTime 2023-04-28 22:25:16
...
(function() {
	...
	// 解析接口菜单框展开的高度
	var developMenuHeight = 150;
	// 解析接口菜单框展开的速度（如果展开动画卡顿请设置0,单位是秒）
	var developMenuSecond = 0.2;
	// 解析接口（可多个）
	const parseInterfaces =["https://player.iizny.com/?url=","https://player.iizny.com/?url=","https://jx.m3u8.tv/jiexi/?url=","https://okjx.cc/?url=","https://m2090.com/?url=","https://www.yemu.xyz/?url=","https://api.okjx.cc:3389/jx.php?url=","https://cv.htoo.vip/?url=","https://jiexi.555jiexi.net:555/player/?url="];
	/*
	 * 非自定义区域
	 * 以下代码勿动
	 * 以下代码勿动
        ...
        try{
            ...
            for(var i in parseInterfaces){
                if(i==parseInterfaces.length-1){
                    html += "<span class='spanStyle' style='"+MenuItemsStyle+"' url='"+parseInterfaces[i]+"'>线路"+(parseInt(i)+1)+"</span>";
                }else{
                    html += "<span class='spanStyle' style='"+MenuItemsStyle+"border-bottom-style:solid;' url='"+parseInterfaces[i]+"'>线路"+(parseInt(i)+1)+"</span>";
                }
            }
            html += "</div></div>";
            div.innerHTML = html;
